...
async function seedDatabase() {
    try {
        ...
        const enginePartsCategory = await Category.create({
            name: 'Engine Parts',
            description: 'Engine components including filters, spark plugs, cooling system, belts and hoses, fuel system'
        });

        const brakeParts = await Category.create({
            name: 'Brake System',
            description: 'Brake pads, rotors, calipers, master cylinders, and brake fluid'
        });

        const suspensionParts = await Category.create({
            name: 'Suspension & Steering',
            description: 'Shock absorbers, springs, control arms, ball joints, and steering components'
        });

        const electricalParts = await Category.create({
            name: 'Electrical & Lighting',
            description: 'Battery, alternator, starter, lights, wipers, and electrical components'
        });

        const transmissionParts = await Category.create({
            name: 'Transmission & Drivetrain',
            description: 'Transmission fluid, clutch components, engine oil, and drivetrain parts'
        });

        const exteriorParts = await Category.create({
            name: 'Exterior Parts',
            description: 'Windscreen, mirrors, bumpers, trim, seals, and weatherstripping'
        });

        const interiorParts = await Category.create({
            name: 'Interior Parts',
            description: 'Seat covers, floor mats, air conditioning, lighting, and interior components'
        });

        const accessoriesParts = await Category.create({
            name: 'Accessories',
            description: 'Customization accessories, door hardware, sensors, and add-on parts'
        });
